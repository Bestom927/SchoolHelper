...
# 最后一个process过程 将string转成embedding 用于训练
# 制作数据集data.npy和label.npy
...
with open('offer_string_list.json','r',encoding='utf8') as f:
     triplet_list = json.load(f)

with open("schoolList.json","r",encoding="utf8") as f:
...
i=0
j=0
gap_total=time.time()-time.time()
for triplet in triplet_list:
    
    T1 = time.time()

    sample_text= triplet["offer_string"]
    label=triplet["school"]
    phrase_list={}
    map_exist=0
    ...
    print ('Sample BERT embedding vector - length', len(sample_sentence_embedding))

    total_data.append(sample_sentence_embedding)
    total_label.append(school_list.index(label))
    print(school_list.index(label),'、',label)
